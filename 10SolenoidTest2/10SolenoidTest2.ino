...
void loop() {
  ...
  // Check if 5 sec have passed since the button press and activate the solenoid
  for (int i = 0; i < 10; i++) {
    if (buttonPressed[i] && (millis() - buttonPressTimes[i] >= 2000)) {
...
void activateSolenoid(int buttonIndex) {
  ...
  delay(15000); // Activate the solenoid for 0.5 second
  digitalWrite(solenoidPins[buttonIndex - 1], LOW); // Deactivate the solenoid
  delay (5000);
}